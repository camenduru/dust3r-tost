...
model = load_model('/content/dust3r/checkpoints/DUSt3R_ViTLarge_BaseDecoder_512_dpt.pth', 'cuda:1')
...
def generate(command):
    ...
    return '/content/dust3r/scene.glb'
...
with gr.Blocks(css=".gradio-container {max-width: 544px !important}", analytics_enabled=False) as demo:
    with gr.Row():
        with gr.Column():
            textbox = gr.Textbox(
                ...
                value="""{
                    "images":[ 
                        {"url":"https://i.ibb.co/rZ8Qvhx/frame01.jpg"}, 
                        {"url":"https://i.ibb.co/p3jQvtn/frame02.jpg"} 
                    ]
                }"""
            )
            button = gr.Button()
    ...
    button.click(fn=generate, inputs=[textbox], outputs=[output_3d], show_progress=True)
